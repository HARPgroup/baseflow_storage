...
baseflow_events <- read_csv("RoanokeTrimStats.csv") |>
  select(Date, Flow, Month, GroupID)
usgs_events <- read_csv("RoanokeGage.csv") |>
...
ggplot(baseflow_events_joined, aes(x = as.factor(Month), y = Flow))+
  ...
  labs(x = "",
       y = "Flow (CFS)",
       title = "Base Flow by Month (Roanoke)")+
  theme_classic()
...
ggplot() +
  ...
  #general formatting
  labs(
    title = "Specific Baseflow Events vs. Historical Monthly Lowflow Percentiles (Roanoke)",
    x = "Month",
    y = "Baseflow (cfs)"
  ) +
  theme_minimal() +
