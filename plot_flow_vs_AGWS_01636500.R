#import the csv
baseflow_trimmed_stats_01636500 <- read_csv("ShenandoahTrimStats.csv")
baseflow_trimmed_stats_01636500 <- baseflow_trimmed_stats_01636500

#Create function to find drainage area (sq.mi) and Flow per drainage area (cfs/sq. mi)  

add_drainage_area <- function(df, gageID) {
  da <- readNWISsite(gageID) %>%
    pull(drain_area_va)
  df %>%
    mutate(
      DrainageArea = da,
      Flow_per_DA = Flow / da
    )
}

baseflow_trimmed_stats_01636500 <- add_drainage_area(
  baseflow_trimmed_stats_01636500,
  "01636500"
)

#Function to convert flow from cfs to inches using the USGS gage area
#flow_to_inches <- function(flow_cfs, drainage_area_sqmi) {
#  flow_cfs * 1.008 / drainage_area_sqmi
#}

#baseflow_trimmed_stats_02055000 <- baseflow_trimmed_stats_02055000 %>%
#  mutate(
#    Flow_inches_day = flow_to_inches(Flow, DrainageArea)
#  )

#Function to convert flow from cfs to icnhes using the USGS gage area
convert.flow <- function(Flow, DrainageArea){
  cfs <- Flow
  #  Create conversion factor
  conversion <- (86400*12)/(5280*5280)
  sp_conv <- conversion/DrainageArea
  
  flow_in <- cfs * sp_conv
  
  return(flow_in)
}
baseflow_trimmed_stats_01636500 <- baseflow_trimmed_stats_01636500 %>% 
  mutate(
    flow_in = convert.flow(Flow, DrainageArea)
  )


#adding AGWS (Active Groundwater Storage) column 
baseflow_trimmed_stats_01636500 <- baseflow_trimmed_stats_01636500 %>% 
  mutate(AGWS = flow_in / (1 - AGWRC))

#Plot flow vs. AGWS For Shenandoah
ggplot(baseflow_trimmed_stats_01636500, aes(x = flow_in, y = AGWS)) +
  geom_point(size = 1.5) +
  labs(
    title = "Flow vs. AGWS For Shenandoah",
    x = "Q (in/day)",
    y = "AGWS"
  ) +
  theme_classic()

#Plot AGWS vs. AGWRC
ggplot(baseflow_trimmed_stats_01636500, aes(x = AGWS, y = AGWRC)) +
  geom_point(size = 1.5) +
  labs(
    title = "AGWS vs. AGWRC For Shenandoah",
    x = "Storage (WS-in.)",
    y = "Recession Coefficient"
  ) +
  theme_classic()