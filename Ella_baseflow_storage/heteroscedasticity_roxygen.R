...
#' @param model an object of class "lm" representing a linear regression of
#' log(Q) vs AGWRC from a series of summarized baseflow events
...
heteroscedasticity <- function(model) {
  ...
  hetero_df <- hetero_df |>
    dplyr::mutate(heteroscedasticity_a_concern = (breusch_pagan_p <= 0.05) |
             (white_test_p <= 0.05)
           )
  return(hetero_df)
}
