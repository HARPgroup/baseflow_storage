...
baseflow_events <- read_csv("LurayTrimStats.csv") |>
  select(Date, Flow, Month, GroupID)
usgs_events <- read_csv("LurayEvent.csv") |>
  select(Date, Flow)
...
ggplot(baseflow_events_joined, aes(x = as.factor(Month), y = Flow))+
  ...
  labs(x = "",
       y = "Flow (CFS)",
       title = "Base Flow by Month (Luray)")+
  theme_classic()
...
ggplot() +
  ...
  #general formatting
  labs(
    title = "Specific Baseflow Events vs. Historical Monthly 
    Lowflow Percentiles (Luray)",
    x = "Month",
    y = "Baseflow (cfs)"
  ) +
  theme_minimal() +
